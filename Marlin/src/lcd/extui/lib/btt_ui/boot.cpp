      ...
      uint32_t i=0;
        uint16_t color=0;
          for(i=0;i<size;i++)//size
          {
            ...
            color=(SPI_ReadWriteByte(W25QXX_DUMMY_BYTE)<<8)|SPI_ReadWriteByte(W25QXX_DUMMY_BYTE);
            W25Qxx_SPI_CS_Set(1); 
...
void ICOlcd_frame_segment_display(uint16_t size, uint32_t addr)
{  
   #if defined(ARDUINO_ARCH_STM32)
      #ifdef __STM32F1__
          W25Qxx_SPI_CS_Set(0);                      
      ...
      SPI_ReadWriteByte((u8)(addr>>8));   
      SPI_ReadWriteByte((u8)(addr));
      SPIReadBuff(display,18050);
      W25Qxx_SPI_CS_Set(1);
           
      SPI4RWLCD_CS0(LOW);
      SPI4RWLCD_RS(HIGH);
      SPIWriteBuff(display,18050);  
      SPI4RWLCD_CS0(HIGH);
      #endif
...
void ICOlcd_frame_display(uint16_t sx, uint16_t sy, uint16_t w, uint16_t h, uint32_t addr)
{
  ...
  {
    segmentSize = (cur+LCD_DMA_MAX_TRANS)<=totalSize ? LCD_DMA_MAX_TRANS : totalSize-cur;
    ICOlcd_frame_segment_display(segmentSize, addr+cur*(2));
  }
}
                ...
                uint16_t mode = 0x0FF0;
                  ...
                  LCD_SetWindow(sx, sy, sx+ICON_WIDTH-1, sy+ICON_HEIGHT-1);
                    ...
                    for(i=0;i<(ICON_WIDTH*ICON_HEIGHT);i++)//size
                    {
                      ...
                      SPI_ReadWriteByte((u8)((addr+i*2)>>8));   
                      SPI_ReadWriteByte((u8)(addr+i*2));   
                      color=((SPI_ReadWriteByte(W25QXX_DUMMY_BYTE)<<8)|SPI_ReadWriteByte(W25QXX_DUMMY_BYTE))& mode;
                      W25Qxx_SPI_CS_Set(1); 
